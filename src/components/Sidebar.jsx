...
const Sidebar = ({expand,setExpand}) => {
  ...
  return (
    <div className={`flex flex-col justify-between bg-[#dbe5f9] pt-7 transition-all z-50 max-md:absolute max-md:h-screen ${expand ? 'p-4 w-64':'md:w-20 w-0 max-md:overflow-hidden'}`}>
      <div>
        ...
        <div className={`mt-8 text-black text-sm ${expand ? "block" : "hidden"}`}>
            ...
            {/* Display a fallback title if chat.title is undefined */}
            {chats && chats.map((chat,index)=>
              <ChatLabel key={index} title={chat.title || "New Chat..."} id={chat._id} openMenu={openMenu} setOpenMenu={setOpenMenu} />
          )}
        </div>
      </div>

      <div>
        <div className={`flex items-center cursor-pointer group relative ${expand ? "gap-1 text-black/80 text-sm p-2.5 border border-primary rounded-lg hover:bg-white/10 cursor-pointer" : "h-10 w-10 mx-auto hover:bg-gray-500/30 rounded-lg"}`}>
      ...
      className={`flex items-center ${expand ? 'hover:bg-white/10 rounded-lg' : 'justify-center w-full'} gap-3 text-black/60 text-sm p-2 mt-2 cursor-pointer`} >
        {
          user ? <UserButton/>
