...
const ChatLabel = ({openMenu,setOpenMenu,id,name}) => {
  ...
  // Dynamic label: prefer `name`, otherwise first message, otherwise fallback
 const displayName =
  name?.trim() ||
  chatData?.autoName ||
  chatData?.messages?.[0]?.content?.slice(0, 30) + "..." ||
  "Untitled Chat";
  ...
  return (
    <div onClick={selectChat} className='flex items-center justify-between p-2 text-black hover:bg-white/10 rounded-lg text-sm group cursor-pointer'>
        <p className='group-hover:max-w-5/6 truncate' >{displayName}</p>
        ...
        <div className={`absolute ${openMenu.id === id && openMenu.open ? 'block' : 'hidden'} -right-36 top-6 bg-gray-700 rounded-xl w-max p-2`} >
            <div onClick={renameHandler} className='flex items-center gap-3 hover:bg-white/10 px-3 py-2 rounded-lg'>
                ...
                <p className='text-white'>Rename</p>
            </div>
