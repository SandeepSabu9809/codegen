            ...
            <div className={`group relative flex max-w-3xl py-3 rounded-xl ${role === 'user' ? 'bg-[#d6d6e3] px-5 ': 'gap-3'} `}>
                ...
                </div>
                {
                    role === 'user' ? (
                        ...
                        <div className="prose prose-md text-black break-words max-w-[80%] p-4 rounded-xl space-y-3">
        <Markdown
  components={{
    pre({ node, ...props }) {
      ...
      return (
        <div className="relative bg-[#090f16] pt-8 pb-2 px-2 rounded-md overflow-hidden">
          {/* Copy button */}
          <button
            // onClick={() => {
    ...
    code({ node, inline, className, children, ...props }) {
      return inline ? (
        <code
          className="bg-[#272727] px-1 py-0.5 rounded text-white"
          {...props}
        >
          {children}
        </code>
